...
function getToken() {
    // In-memory token (session mode) takes priority
    if (process.env.SS_TOKEN) return process.env.SS_TOKEN;
...
function tokenStatus() {
    // In-memory session token — no expiry tracked locally, report as valid
    if (process.env.SS_TOKEN) {
        return {
            valid: true,
            expiresAt: null,
            savedAt: null,
    ...
    try {
        ...
        return { valid: false, expiresAt: null, savedAt: null, minutesLeft: 0, source: 'none' };
    }
}
...
module.exports = { TOKEN_FILE, saveToken, getToken, requireToken, tokenStatus, deleteToken };
