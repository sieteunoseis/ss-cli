...
async function resolveSecret(baseUrl, apiToken, target) {
    ...
    // Get SSH template IDs from config (e.g. "6007,6010")
    ...
    if (templateIds.length === 0) {
        console.error(`No sshTemplates configured. Run: ss-cli config set sshTemplates 6007,6010`);
    ...
    if (allResults.length === 0) {
        throw new Error(`No SSH secrets found matching "${searchTerm}". Check sshTemplates config or use secret ID.`);
    }
