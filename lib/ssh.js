...
const SSH_CACHE_FILE = path.join(CONFIG_DIR, 'ssh-cache.json');

function loadSshCache() {
    try {
        return JSON.parse(fs.readFileSync(SSH_CACHE_FILE, 'utf8'));
    } catch {
        return {};
    }
}

function saveSshCache(cache) {
    fs.mkdirSync(CONFIG_DIR, { recursive: true });
    fs.writeFileSync(SSH_CACHE_FILE, JSON.stringify(cache, null, 2) + '\n');
}

function getCachedSecretId(hostname) {
    const cache = loadSshCache();
    return cache[hostname.toLowerCase()] || null;
}

function setCachedSecretId(hostname, secretId) {
    const cache = loadSshCache();
    cache[hostname.toLowerCase()] = secretId;
    saveSshCache(cache);
}
...
async function resolveSecret(baseUrl, apiToken, target) {
    ...
    // Check cache first
    const cachedId = getCachedSecretId(target);
    if (cachedId) {
        try {
            const secret = await getSecret(baseUrl, apiToken, cachedId);
            console.error(`Cached: [${secret.id}] ${secret.name}`);
            return secret;
        } catch {
    ...
    if (templateIds.length === 0 && !folderId) {
        ...
        const secret = await resolveFromResults(baseUrl, apiToken, await searchSecrets(baseUrl, apiToken, searchTerm), searchTerm);
        setCachedSecretId(target, secret.id);
        return secret;
    }
    ...
    const sshDomain = getConfigValue('sshDomain');
    const exact = allResults.find(r => {
        const name = r.name.toLowerCase();
        if (name === target.toLowerCase()) return true;
        if (name === searchTerm.toLowerCase()) return true;
        if (sshDomain && name === `${searchTerm}.${sshDomain}`.toLowerCase()) return true;
        return false;
    });
    if (exact) {
...
async function listAllSshSecrets(baseUrl, apiToken) {
    ...
    // Unlike searchSshSecrets (used for narrowing an ambiguous hostname search,
    // where a broad folder+templates candidate pool is fine because exact-name
    // matching narrows it afterward), listing needs a clean, browsable result:
    // sshTemplates alone can match secrets anywhere on the server that happen to
    // reuse an SSH template for unrelated things (license keys, wildcard certs).
    // So once sshFolder is set, scope to it exclusively; only fall back to
    // sshTemplates when no folder is configured.
    let results;
    if (folderId) {
        results = await searchSecrets(baseUrl, apiToken, '', folderId, null, 50, true);
    } else {
        results = [];
        const seen = new Set();
        for (const tid of templateIds) {
            const tidResults = await searchSecrets(baseUrl, apiToken, '', null, tid);
            tidResults.forEach(r => { if (!seen.has(r.id)) { seen.add(r.id); results.push(r); } });
        }
    }
...
async function runSshList(baseUrl, apiToken, useAll, sshArgs) {
    ...
    const secretId = await prompt.run().catch(() => null);
    if (!secretId) return 130;
